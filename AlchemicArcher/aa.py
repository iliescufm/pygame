...
Font_1 = {'A':[6],'B':[6],'C':[6],'D':[6],'E':[6],'F':[6],'G':[6],'H':[6],'I':[6],'J':[6],'K':[6],'L':[6],'M':[6],'N':[6],'O':[6],'P':[6],'Q':[6],'R':[6],'S':[6],'T':[6],'U':[6],'V':[6],'W':[6],'X':[6],'Y':[6],'Z':[6],
          '.':[3],'-':[6],',':[3],':':[3],'+':[6],'\'':[3],
          'a':[6],'b':[6],'c':[6],'d':[6],'e':[6],'f':[5],'g':[6],'h':[6],'i':[2],'j':[5],'k':[5],'l':[3],'m':[6],'n':[5],'o':[6],'p':[6],'q':[6],'r':[5],'s':[5],'t':[5],'u':[5],'v':[6],'w':[6],'x':[6],'y':[6],'z':[6],
          '!':[2],'?':[6],
          '0':[6],'1':[3],'2':[6],'3':[6],'4':[6],'5':[6],'6':[6],'7':[6],'8':[6],'9':[6]}
...
x_buttons = [pygame.image.load('Data/Images/Misc/button_x.png').convert(),pygame.image.load('Data/Images/Misc/button_x_2.png').convert()]
z_buttons = [pygame.image.load('Data/Images/Misc/button_z.png').convert(),pygame.image.load('Data/Images/Misc/button_z_2.png').convert()]
...
AnimationData = {
    'player':{5:[1,1,'player_walking2'],4:[1,1,'player_walking'],2:[3,3,'player_walking'],3:[3,3,'player_walking2']},
    'bow':{0:[3,4,'bow'],1:[3,4,'bow2']},
    'blob':{0:[1,1,'blob_start'],1:[4,3,'blob_start'],2:[4,3,'blob_start2'],3:[3,3,'blob_moving'],4:[3,3,'blob_moving2']},
    'snail':{0:[23,30,'snail']},
...
while True:
    ...
    if random.randint(1,(7-wind_strength)*2) == 1:
        ...
        Wind[2].append([240,random.randint(0,160),random.randint(4+int(wind_strength/2),6+int(wind_strength/2)),random.randint(4,16)])
    if random.randint(1,(7-wind_strength)*2) == 1:
        Wind[random.randint(2,5)].append([240,random.randint(0,160),random.randint(4+int(wind_strength/2),6+int(wind_strength/2)),random.randint(4,16)])
    for Layer in Wind:
    ...
    for item in Items:
        ...
        if item[0] in AnimationData:
            ...
            if item[4] >= AnimationData[item[0]][0][0]:
                item[4] = 0
            DisplaySurf.blit(Animations[AnimationData[item[0]][0][2]][item[4]],(item[1]+ScrollX,item[2]+ScrollY+1))
        else:
            ...
            for NPC in NPCs:
                if NPC.obj.rect.colliderect(arrow_obj.rect):
                    ...
                    try:
                        ...
                        pass
                    AddParticles(Particles,NPC.x+entity_sizes[NPC.type][0]/2,NPC.y+entity_sizes[NPC.type][1]/2,[NPCparticles[NPC.type]],(0.5,1),(7,9),75,'fall',0.15)
        DisplaySurf.blit(img,(Arrow[0]+ScrollX,Arrow[1]+ScrollY))
        if Arrow[3] != None:
            if random.randint(1,2) == 1:
                if Arrow[2] == 'r':
                    if collisions[2] == False:
                        Particles.append([Arrow[0]+7,Arrow[1]+1,0.5,-0.5,Potions[Arrow[3]][0],1,0,20,'normal',1])
                if Arrow[2] == 'l':
                    if collisions[3] == False:
                        Particles.append([Arrow[0],Arrow[1]+1,-0.5,-0.5,Potions[Arrow[3]][0],1,0,20,'normal',1])
        try:
